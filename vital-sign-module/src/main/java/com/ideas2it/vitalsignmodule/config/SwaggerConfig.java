package com.ideas2it.vitalsignmodule.config;
...
public class SwaggerConfig {
	...
	public Docket postsApi() {
		return new Docket(DocumentationType.SWAGGER_2).groupName("vitalsign-api")
				.apiInfo(apiInfo()).select().paths(postPaths()).build();
	}

	private com.google.common.base.Predicate<String> postPaths() {
		return or(regex("/vitalsign.*"), regex("/vital.*"));
	}

	private ApiInfo apiInfo() {
		return new ApiInfoBuilder().title("Patient API")
				.description("Vital Informatiom")
